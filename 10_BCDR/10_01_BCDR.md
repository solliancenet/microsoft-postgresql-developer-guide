...
For more information on how to utilize replication in Azure Database for PostgreSQL Flexible Server, reference [Logical replication and logical decoding in Azure Database for PostgreSQL - Flexible Server](https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/concepts-logical).
...
To alleviate the challenges of region and cross-region failover scenerios, Azure Database for PostgreSQL Flexible Server included the ability to create **Virtual Endpoints**.  Virtual Endpoints are read-write and read-only listener endpoints, that remain consistent irrespective of the current role of the PostgreSQL instance. This means you don't have to update your application's connection string after performing the promote to primary server action, as the endpoints will automatically point to the correct instance following a role change.
