...
If the application is made up of many different client instances around the world, it may not be feasible to update all of the clients. A load balancer may assist in this challenge.
...
To alleviate the challenges of region and cross-region failover scenerios, Azure Database for PostgreSQL included the ability to create **Virtual Endpoints**.  These are a type of load balancer solution built into the product to allow for failover scenerios without having to touch each of your clients.
