...
>![Watch icon](media/watch.png "Watch") **Watch:** [High Availability [9 of 16] | Azure Database for PostgreSQL Flexible Server - Beginners Series](https://www.youtube.com/watch?v=5hp8KlNNPpA&list=PLlrxD0HtieHghqeFLMwaGxfxPbndt52Ap&index=9)

![This image demonstrates Zone-Redundant HA for PostgreSQL Flexible Server.](media/1-flexible-server-overview-zone-redundant-ha.png "Zone-Redundant HA")
...
![This image demonstrates HA for PostgreSQL Flexible Server in a single zone.](./media/flexible-server-overview-same-zone-ha.png "HA in a single zone")
...
To learn more about HA with PostgreSQL Flexible Server, consult the [documentation.](https://learn.microsoft.com/azure/postgresql/flexible-server/flexible-server/concepts-high-availability)
...
>![Watch icon](media/watch.png "Watch") **Watch:** [Replication [10 of 16] | Azure Database for PostgreSQL Flexible Server - Beginners Series](https://www.youtube.com/watch?v=K-MScJu4Jqw&list=PLlrxD0HtieHghqeFLMwaGxfxPbndt52Ap&index=10)
...
Often, developers use load balancers, like ProxySQL, to direct read operations to read replicas automatically. ProxySQL can [run on an Azure VM](https://techcommunity.microsoft.com/t5/azure-database-for-PostgreSQL-blog/load-balance-read-replicas-using-proxysql-in-azure-database-for/ba-p/880042) or [Azure Kubernetes Service.](https://techcommunity.microsoft.com/t5/azure-database-for-PostgreSQL-blog/deploy-proxysql-as-a-service-on-kubernetes-using-azure-database/ba-p/1105959)
...
- [Azure Portal](https://learn.microsoft.com/azure/postgresql/flexible-server/flexible-server/how-to-read-replicas-portal)
- [Azure CLI](https://learn.microsoft.com/azure/postgresql/flexible-server/flexible-server/how-to-read-replicas-cli)
...
>![Watch icon](media/watch.png "Watch") **Watch:** [Backup and Restore [8 of 16] | Azure Database for PostgreSQL Flexible Server - Beginners Series](https://www.youtube.com/watch?v=B7qGSX8IeSY&list=PLlrxD0HtieHghqeFLMwaGxfxPbndt52Ap&index=8)
...
Azure Database for PostgreSQL Flexible Server takes automatic backups of data and transaction log files. These backups can be stored in locally-redundant storage (replicated multiple times in a datacenter); zone-redundant storage (multiple copies are stored in two separate availability zones in a region); and geo-redundant storage (multiple copies are stored in two separate Azure regions).

Azure Database for PostgreSQL Flexible Server supports automatic backups for 7 days by default. It may be appropriate to modify this to the current maximum of 35 days. It is important to be aware that if the value is changed to 35 days, there will be charges for any extra backup storage over 1x of the storage allocated. Data file backups are taken once daily, while transaction log backups are taken every five minutes.

Find additional storage pricing details for Flexible Server [here.](https://azure.microsoft.com/pricing/details/PostgreSQL/flexible-server/)

There are several current limitations to the database backup feature as described in the [Backup and restore in Azure Database for PostgreSQL Flexible Server](https://learn.microsoft.com/azure/postgresql/flexible-server/concepts-backup) docs article. It is important to understand them when deciding what additional strategies should be implemented.
...
>![Note icon](media/note.png "Note") **Note:** [Some regions](https://learn.microsoft.com/azure/postgresql/flexible-server/concepts-pricing-tiers#storage) do not yet support storage up to 16TB.
...
During a database restore, any supporting items outside of the database will also need to be restored.  Review the migration process. See [Perform post-restore tasks](https://learn.microsoft.com/azure/postgresql/flexible-server/concepts-backup#perform-post-restore-tasks) for more information.
...
Learn more about backup and restore in Flexible Server from the [Microsoft documentation.](https://learn.microsoft.com/azure/postgresql/flexible-server/flexible-server/concepts-backup-restore)
